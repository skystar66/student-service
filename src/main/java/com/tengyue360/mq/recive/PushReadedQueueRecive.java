...
public class PushReadedQueueRecive {
    ...
    private static Logger logger = LoggerFactory.getLogger(PushReadedQueueRecive.class);
    @Autowired
    ...
    public void process(@Payload MessageTemplate messageTemplate1, Channel channel, Message message) throws IOException {
        logger.info("队列：{},收到消息：{}", QueueConstant.QUEUE_MESSAGE_PUSH_READED_MESSAGE, messageTemplate1);
        try {
            //推送消息
